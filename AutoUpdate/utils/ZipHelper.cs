...
namespace AutoUpdate.utils
{
    class ZipHelper
    {
        ...
        public static bool ZipFilesInDirectory(string folderToZip, string zipedFile, string password)
        {
            ...
            result = ZipFilesInDirectory(folderToZip, zipStream, "");
        ...
        private static bool ZipFilesInDirectory(string folderToZip, ZipOutputStream zipStream, string parentFolderName)
        {
            ...
            try
            {
                ...
                foreach (string file in files)
                {
                    ...
                    ent = new ZipEntry(Path.Combine(parentFolderName + Path.GetFileName(file))); // , Path.GetFileName(folderToZip) + "/" 
        ...
        public static bool UnZip(string fileToUnZip, string zipedFolder, string password)
        {
            ...
            try
            {
                ...
                while ((ent = zipStream.GetNextEntry()) != null)
                {
                    if (!string.IsNullOrEmpty(ent.Name))
                    {
                        ...
                        using (fs = new FileStream(fileName, FileMode.OpenOrCreate, FileAccess.Write, FileShare.ReadWrite))// ;
                        {
                            UpdateLog.WriteLogLine("解压："+fileName);
                            int size = 2048;
            ...
            {
                UpdateLog.WriteLogLine("解压出现异常:"+e.Message+"---"+e.StackTrace+"---"+e.TargetSite);
                result = false;


                //String fn = Environment.CurrentDirectory + "\\SM.exe";
                //UpdateLog.WriteLogLine("文件路径:"+fn);
                //Process tool = new Process();
                //tool.StartInfo.FileName = "handle.exe";
                //tool.StartInfo.Arguments = fn + " /accepteula";
                //tool.StartInfo.UseShellExecute = false;
                //tool.StartInfo.RedirectStandardOutput = true;
                //tool.Start();
                //tool.WaitForExit();
                //string outputTool = tool.StandardOutput.ReadToEnd();
                //UpdateLog.WriteLogLine("进程信息查找:" + outputTool);
        ...
        private static string GetMD5HashFromFile(string fileName)
        {
            ...
            {
                UpdateLog.WriteLogLine("获取Md5值失败:" +ex.Message);
                return null;
            }
        }
        #endregion
